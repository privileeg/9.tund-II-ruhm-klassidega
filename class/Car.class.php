<?php class Car{
	...
	function getAll($q, $sort, $direction) {
		...
		if($q == ""){
			...
			("SELECT id, plate, color FROM 
			cars_color WHERE deleted IS NULL ORDER BY $sort $orderBy");
		} else {
			
			echo "Otsib: ".$q;
			
			//teen otsi s6na lisan m6lemale poole $
			
			$searchword = "%".$q."$";
			
			$stmt = $this->connection->prepare
			("SELECT id, plate, color FROM cars_color 
			WHERE deleted IS NULL AND (plate LIKE ? 										
			OR color LIKE ?)ORDER BY $sort $orderBy");	
